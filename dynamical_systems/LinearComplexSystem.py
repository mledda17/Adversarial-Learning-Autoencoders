...
class LinearSystemComplexStable(DynamicalSystem):
    def __init__(self, state_size=3, input_size=1, output_size=1, non_linear_input=False, sigma=0.02):
        ...
        # Matrice A con autovalori complessi coniugati e modulo < 1
        # Forma: blocco rotazione + terzo stato reale stabile
        self.A = np.array([[0.95, -0.1, 0],
                           [0.1,  0.95, 0],
                           [0,    0,   0.9]])
        
        self.B = np.array([[1.0, 0.5, 0.0]]).T  # Input sul primo e secondo stato
        self.C = np.array([[1, 0, 0]])  # Output solo sul primo stato
        self.sigma = sigma
    ...
    def system_dynamics(self, dim):
        ...
        for i in range(dim):
            ...
            y_n[i] = self.output_map(x_k)
            x_k = self.state_map(x_k, np.reshape(u_k, (self.input_size, 1)))
            u_n[i] = u[0]
